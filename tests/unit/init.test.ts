...
describe("buildTemplate", () => {
  it("ts template imports definePumpConfig and has the three built-in types", () => {
    ...
    expect(t).toContain(`import { definePumpConfig } from 'pumpp-cli'`);
  ...
  it("mjs template uses JSDoc type hint and no TS import", () => {
    ...
    expect(t).toContain(`@type {import('pumpp-cli').PumpInputConfig}`);
...
describe("runInit", () => {
  ...
  it("refuses to overwrite without --force", async () => {
    await fs.writeFile(
      path.join(tmp, "pumpp.config.ts"),
      "/* existing */",
      "utf8",
    );
    await expect(
      runInit({ cwd: tmp, format: "ts", force: false }),
